...
export class HeadLayerOutput extends Struct({
    output: Provable.Array(Provable.Array(Provable.Array(UInt32, NFILTERS), OUTCOL), OUTROWS)
}) { }
...
export async function headLayerInferece(
    ...
    let output  = Array<Array<Array<UInt32>>>(OUTROWS).fill(
        Array<Array<UInt32>>(OUTCOL).fill(
            Array<UInt32>(NFILTERS).fill(ZERO)
        )
    );
